...
const AddPlayerForm = () => {
  ...
  const validationSchema = Yup.object({
    ...
    height: Yup.number().required('Height is required').min(160, "Dwarves dont play in NBA").max(240, "This aint robert wadlow"),
    position: Yup.string().required('Position is required'),
    teamNumber: Yup.number().required('Team Number is required').min(0, "Player cannot have a negative number").max(99, "Player cannot have number above 100"),
    pictureLink: Yup.string().url('Invalid URL').required('Picture Link is required'),
    fantasyScore: Yup.number().min(0,"Must be greater or equal to 0").max(100, "cant be higher than 100"),
    statistics: Yup.object().shape({
      ...
      freeThrowPercentage: Yup.number().min(0, "Cant go into negatives").max(100, "Percentage ends at 100"),
    }),
  });
  ...
  return (
    ...
    <form onSubmit={formik.handleSubmit}>
      ...
      <div>
        ...
        {formik.touched.birthdate && formik.errors.birthdate && <div className={styles.error}>{formik.errors.birthdate}</div>}
      </div>
      ...
      <div>
        ...
        {formik.touched.height && formik.errors.height && <div className={styles.error}>{formik.errors.height}</div>}
      </div>
  
      <div>
        <label htmlFor="team">Team:</label>
        <input type="text" id="team" {...formik.getFieldProps('team')} disabled />
        {formik.touched.team && formik.errors.team && <div className={styles.error}>{formik.errors.team}</div>}
      </div>
      ...
      <div>
        ...
        <input type="number" id="teamNumber" {...formik.getFieldProps('teamNumber')} disabled />
      ...
      <div>
        ...
        <input type="number" id="fantasyScore" {...formik.getFieldProps('fantasyScore')} />
      ...
      <div>
        ...
        <input type="number" id="statistics.gamesPlayed" {...formik.getFieldProps('statistics.gamesPlayed')} />
        {formik.touched['statistics.gamesPlayed'] && formik.errors['statistics.gamesPlayed'] && (
          <div className={styles.error}>{formik.errors['statistics.gamesPlayed']}</div>
        )}
      </div>
  
      <div>
        <label htmlFor="statistics.rebounds">Rebounds:</label>
        <input type="number" id="statistics.rebounds" {...formik.getFieldProps('statistics.rebounds')} />
        {formik.touched['statistics.rebounds'] && formik.errors['statistics.rebounds'] && (
      ...
      <div>
        ...
        <input type="number" id="statistics.points" {...formik.getFieldProps('statistics.points')} />
      ...
      <div>
        ...
        <input type="number" id="statistics.foulsCommitted" {...formik.getFieldProps('statistics.foulsCommitted')} />
        {formik.touched['statistics.foulsCommitted'] && formik.errors['statistics.foulsCommitted'] && (
          <div className={styles.error}>{formik.errors['statistics.foulsCommitted']}</div>
        )}
      </div>
  
      <div>
        <label htmlFor="statistics.freeThrowsMade">Free Throws Made:</label>
        <input type="number" id="statistics.freeThrowsMade" {...formik.getFieldProps('statistics.freeThrowsMade')} />
        {formik.touched['statistics.freeThrowsMade'] && formik.errors['statistics.freeThrowsMade'] && (
          <div className={styles.error}>{formik.errors['statistics.freeThrowsMade']}</div>
        )}
      </div>
  
      <div>
        <label htmlFor="statistics.freeThrowPercentage">Free Throw Percentage:</label>
        <input type="number" id="statistics.freeThrowPercentage" {...formik.getFieldProps('statistics.freeThrowPercentage')} />
        {formik.touched['statistics.freeThrowPercentage'] && formik.errors['statistics.freeThrowPercentage'] && (
          <div className={styles.error}>{formik.errors['statistics.freeThrowPercentage']}</div>
        )}
      </div>
