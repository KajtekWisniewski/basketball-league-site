...
class TeamHandler {
  ...
  async getTeamsByConferenceOrDivision(conferenceOrDivision) {
    try {
      const teams = await Team.find({
        $or: [{ conference: conferenceOrDivision }, { division: conferenceOrDivision }]
      }).populate('roster');
      return teams;
    } catch (error) {
  ...
  async updateAllPlayersInTheRosterToTeamless(teamId) {
    try {
      const team = await Team.findById(teamId);

      if (team) {
        // Update all players in the roster array to "teamless"
      ...
      console.error('Error updating all players in the roster to teamless:', error);
      throw error;
    }
  }
